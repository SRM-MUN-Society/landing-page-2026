...
export default function GlyphPortal({
  ...
  useLayoutEffect(() => {
    ...
    const buttons = Array.from(
      choices.querySelectorAll<HTMLButtonElement>("button")
    );
    ...
    const select = (next: Ink | null) => {
      ...
      for (const button of buttons) {
        const selected =
          Number(button.dataset.gpLetter) === target?.index;
      ...
      for (const option of Array.from(picker.options))
        option.disabled =
          option.value === "" ||
          !candidates.some((c) => c.index === Number(option.value));
    ...
    const paint = (progress: number) => {
      ...
      const cx =
        center.x + ((target?.x ?? center.x) - center.x) * blend;
      const cy =
        center.y + ((target?.y ?? center.y) - center.y) * blend;
      const roll =
        -4 * smooth(0.06, 0.5, t) * (1 - smooth(0.62, 0.92, t));
    ...
    const layout = () => {
      ...
      W = pin.clientWidth;
      const smallViewport =
        section.querySelector<HTMLElement>("[data-gp-viewport]")!.offsetHeight;
      ...
      const wordHeight =
        hasFront && H < 480
          ? Math.min(H * 0.38, Math.max(24, H - 264))
          : H * 0.38;
      startScale = Math.min(
        (W * 0.84) / bounds.width,
  ...
  return (
    ...
    >
      <style suppressHydrationWarning>{`
        ${q}{--gp-paper:#fff;--gp-ink:#0c0c0c;--gp-field:#0a0a0a;--gp-foreground:#fbfbfa;position:relative;isolation:isolate;background:var(--gp-paper);color:var(--gp-ink);font-family:Arial,sans-serif;}
        ...
        ${q} [data-gp-select]{min-height:44px;min-width:90px;border:1px solid #d4d4d4;border-radius:4px;background:var(--gp-paper);color:var(--gp-ink);padding:0 10px;font:inherit;}
        ...
        ${q} [data-gp-content][data-gp-empty]{min-height:0;padding:0;background:transparent;}
          ...
          {background ?? (
            <div
              ...
              style={{
                ...
                background:
                  "radial-gradient(circle at 18% 8%, rgba(80,80,80,.55), transparent 34%), radial-gradient(circle at 82% 20%, rgba(251,251,250,.1), transparent 28%), radial-gradient(circle at 48% 78%, rgba(0,0,0,.5), transparent 44%), linear-gradient(135deg,#1a1a1a 0%,#0f0f0f 48%,#050505 100%)",
              }}
            />
          )}
        </div>
